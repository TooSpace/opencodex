...
describe("Codex catalog sync hardening", () => {
  ...
  test("Gap B: drops legacy OpenAI-family natives but keeps supported + user natives", () => {
    ...
    writeFileSync(catalogPath, JSON.stringify({
      models: [
        ...
        nativeEntry("gpt-5.3-codex", 104),
        nativeEntry("gpt-5.2", 104),
        nativeEntry("codex-auto-review", 104),
        nativeEntry("user-native", 10),
      ],
    }, null, 2) + "\n");
    ...
    expect(slugs).not.toContain("gpt-5.2");
  ...
  test("native-alias suppression preserves authoritative metadata on account-qualified rows", () => {
    ...
    const r = runScript(codexHome, opencodexHome, `
      ...
      const config = {
        ...
        providers: {
          openai: { adapter: "openai-responses", baseUrl: "https://chatgpt.com/backend-api/codex", liveModels: false },
          Nova1: { adapter: "openai-chat", baseUrl: "https://api.example.test/v1", liveModels: false, models: ["codex/gpt-5.6-sol"] }
        },
    ...
    expect(rows.filter(row => row.slug === "gpt-5.6-sol")).toEqual([
      expect.objectContaining({ display_name: "Nova Sol", opencodex_catalog_kind: "combo-native-alias-v1" }),
    ]);
  ...
  test("account rows reconcile idempotently and independently from authoritative provider empties", () => {
    ...
    writeFileSync(catalogPath, JSON.stringify({ models: [
      { ...nativeEntry("gpt-5.5", 0), comp_hash: "native-5.5-hash", base_instructions: "Native 5.5 instructions", model_messages: { instructions_template: "Native 5.5 instructions" }, tool_mode: null, context_window: 128_000, max_context_window: 128_000, auto_compact_token_limit: 115_200 },
      { ...nativeEntry("gpt-5.4", 1), comp_hash: "native-5.4-hash", base_instructions: "Native 5.4 instructions", model_messages: { instructions_template: "Native 5.4 instructions" }, tool_mode: "code_mode_only" },
      nativeEntry("gpt-5.4-mini", 2),
      routedEntry("vendor/stable-model", 5),
      { ...routedEntry("foreign/gpt-5.5", 6), description: "Foreign provider description" },
      { ...routedEntry("team/gpt-5.5", 7), display_name: "Stale provider row with a colliding slug" },
      { ...nativeEntry("removed/gpt-5.5", 8), description: "Retired generated row", opencodex_catalog_kind: accountMarker },
    ] }, null, 2) + "\n");
    const r = runScript(codexHome, opencodexHome, `
      ...
      const config = {
        providers: { openai: { adapter: "openai-responses", baseUrl: "https://chatgpt.com/backend-api/codex", liveModels: false } },
        codexAccounts: [{ id: "stored-team-account", email: "private@example.test", alias: "Private Display Name", isMain: false }],
        codexAccountNamespaces: { desktop: "@main", team: "stored-team-account", removed: "missing-account" }
      };
    ...
    const rows = JSON.parse(readFileSync(catalogPath, "utf8")).models as Array<any>;
    ...
    expect(firstBare).toMatchObject({ context_window: 272_000, max_context_window: 272_000, auto_compact_token_limit: 244_800 });
    expect(firstTeam).toMatchObject({ context_window: firstBare?.context_window, max_context_window: firstBare?.max_context_window, auto_compact_token_limit: firstBare?.auto_compact_token_limit });
    expect(rows.some((row: any) => row.slug === "vendor/stable-model")).toBe(true);
    expect(rows.some((row: any) => row.slug === "foreign/gpt-5.5")).toBe(true);
    expect(rows.some((row: any) => row.slug === "removed/gpt-5.5")).toBe(false);
    expect(rows.find((row: any) => row.slug === "gpt-5.5")?.visibility).toBe("hide");
    expect(rows.find((row: any) => row.slug === "desktop/gpt-5.5")?.visibility).toBe("list");
    const bare = rows.find((row: any) => row.slug === "gpt-5.5");
    const team = rows.find((row: any) => row.slug === "team/gpt-5.5");
    expect(team).toMatchObject({ display_name: "team / 5.5", opencodex_catalog_kind: accountMarker, comp_hash: "native-5.5-hash", visibility: "list" });
    expect(team?.description).toBe(bare?.description);
    expect(rows.filter((row: any) => row.slug === "team/gpt-5.5")).toHaveLength(1);
    for (const selector of ["desktop", "team"]) {
      expect(rows.some((row: any) => row.slug === `${selector}/gpt-5.4`)).toBe(true);
      expect(rows.some((row: any) => row.slug === `${selector}/gpt-5.4-mini`)).toBe(true);
    }
    for (const nativeSlug of ["gpt-5.5", "gpt-5.4"]) {
      const native = rows.find((row: any) => row.slug === nativeSlug);
      const qualified = rows.find((row: any) => row.slug === `team/${nativeSlug}`);
      expect(qualified).toMatchObject({ comp_hash: native?.comp_hash, base_instructions: native?.base_instructions, model_messages: native?.model_messages, tool_mode: native?.tool_mode });
    }
  ...
  test("account sync preserves an observed account-only native id without creating a bare row", () => {
    ...
    const r = runScript(codexHome, opencodexHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: { openai: { adapter: "openai-responses", baseUrl: "https://chatgpt.com/backend-api/codex", liveModels: false } }, codexAccountNamespaces: { team: "@main" } }).then(res => console.log(JSON.stringify(res)));
    `);
    expect(r.status).toBe(0);
  ...
  test("a live provider row shadowed by an account selector warns once per runtime generation", () => {
    ...
    const r = runScript(codexHome, opencodexHome, `
      ...
      const config = { providers: { openai: { adapter: "openai-responses", baseUrl: "https://chatgpt.com/backend-api/codex", liveModels: false }, team: { adapter: "openai-chat", baseUrl: "https://api.example.test/v1", liveModels: false, models: ["gpt-5.5"] } }, codexAccounts: [{ id: "stored-team-account", isMain: false }], codexAccountNamespaces: { team: "stored-team-account" } };
      syncCatalogModels(config).then(() => syncCatalogModels(config)).then(() => { resetCatalogRuntimeStateForTests(); return syncCatalogModels(config); }).then(res => console.log(JSON.stringify(res)));
    `);
    expect(r.status).toBe(0);
    expect((r.stderr.match(/account selector collision on "team\/gpt-5\.5"/g) ?? []).length).toBe(2);
    const rows = JSON.parse(readFileSync(catalogPath, "utf8")).models as Array<{ slug: string; opencodex_catalog_kind?: string }>;
    expect(rows.filter(row => row.slug === "team/gpt-5.5")).toEqual([expect.objectContaining({ opencodex_catalog_kind: "account-selector-v1" })]);
  });
  ...
  test("non-OpenAI-only sync omits account rows without reprioritizing routed models", () => {
    ...
    const r = runScript(codexHome, opencodexHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: { mock: { adapter: "openai-chat", baseUrl: "https://api.example.test/v1", liveModels: false, models: ["static-model"] } }, codexAccountNamespaces: { desktop: "@main" } }).then(res => console.log(JSON.stringify(res)));
    `);
    expect(r.status).toBe(0);
    const rows = JSON.parse(readFileSync(catalogPath, "utf8")).models as Array<{ slug: string; priority?: number }>;
  ...
  test("catalog sync persists routed code mode without changing native account rows", () => {
    ...
    const r = runScript(codexHome, opencodexHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: { openai: { adapter: "openai-responses", baseUrl: "https://chatgpt.com/backend-api/codex", liveModels: false }, deepseek: { adapter: "openai-responses", baseUrl: "https://api.example.test/v1", liveModels: false, models: ["deepseek-v4-flash"] } }, codexAccounts: [{ id: "stored-team-account", isMain: false }], codexAccountNamespaces: { team: "stored-team-account" } }).then(res => console.log(JSON.stringify(res)));
    `);
    expect(r.status).toBe(0);
    const rows = JSON.parse(readFileSync(catalogPath, "utf8")).models as Array<{ slug: string; tool_mode?: string | null }>;
    expect(rows.find(row => row.slug === "deepseek/deepseek-v4-flash")?.tool_mode).toBe("code_mode_only");
  ...
  test("disabled canonical OpenAI keeps bare bootstrap rows but omits unrouteable account rows", () => {
    ...
    const r = runScript(codexHome, opencodexHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: { openai: { adapter: "openai-responses", baseUrl: "https://chatgpt.com/backend-api/codex", disabled: true, liveModels: false } }, codexAccounts: [{ id: "stored-side-account", isMain: false }], codexAccountNamespaces: { team: "stored-side-account" } }).then(res => console.log(JSON.stringify(res)));
    `);
    expect(r.status).toBe(0);
    const rows = JSON.parse(readFileSync(catalogPath, "utf8")).models as Array<{ slug: string; visibility?: string }>;
  ...
  test("native model fallback remains reachable without a live catalog", () => {
    writeFileSync(join(codexHome, "config.toml"), 'model_catalog_json = "missing-catalog.json"\n', "utf8");
    const r = runScript(codexHome, opencodexHome, `
  ...
  test("account sync recovers supported natives that were hidden before selectors existed", () => {
    ...
    const r = runScript(codexHome, opencodexHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: { openai: { adapter: "openai-responses", baseUrl: "https://chatgpt.com/backend-api/codex", liveModels: false } }, disabledModels: ["gpt-5.4", "team/gpt-5.5"], codexAccounts: [{ id: "stored-side-account", isMain: false }], codexAccountNamespaces: { desktop: "@main", team: "stored-side-account" } }).then(res => console.log(JSON.stringify(res)));
    `);
    expect(r.status).toBe(0);
    const rows = JSON.parse(readFileSync(catalogPath, "utf8")).models as Array<{ slug: string; visibility?: string; opencodex_catalog_kind?: string }>;
    expect(rows.find(row => row.slug === "gpt-5.5")?.visibility).toBe("hide");
    expect(rows.find(row => row.slug === "team/gpt-5.5")).toMatchObject({ visibility: "hide", opencodex_catalog_kind: "account-selector-v1" });
    expect(rows.find(row => row.slug === "desktop/gpt-5.5")).toMatchObject({ visibility: "list", opencodex_catalog_kind: "account-selector-v1" });
  ...
  test("a poisoned routed row already on disk is repaired, not dropped, by the next sync", () => {
    const catalogPath = join(codexHome, "catalog.json");
    writeFileSync(join(codexHome, "config.toml"), 'model_catalog_json = "catalog.json"\n', "utf8");
    const poisoned = { ...routedEntry("zenmux/meta-muse-spark-1.1", 5), input_modalities: ["text", "image", "video"] };
    writeFileSync(catalogPath, JSON.stringify({ models: [nativeEntry("gpt-5.5", 0), poisoned] }, null, 2) + "\n");
    const r = runScript(codexHome, opencodexHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: {} }).then(res => console.log(JSON.stringify(res)));
    `);
    expect(r.status).toBe(0);
    const written = JSON.parse(readFileSync(catalogPath, "utf8")) as { models: Array<{ slug: string; input_modalities?: unknown }> };
    const row = written.models.find(m => m.slug === "zenmux/meta-muse-spark-1.1");
    expect(row).toBeDefined();
    expect(row!.input_modalities).toEqual(["text", "image"]);
    const outOfEnum = written.models.flatMap(m => Array.isArray(m.input_modalities) ? (m.input_modalities as unknown[]).filter(v => v !== "text" && v !== "image" && v !== "audio") : []);
  ...
  test("drops OpenCodex-authored rows of a deleted provider, keeps foreign rows", () => {
    ...
    const r = runScript(codexHome, opencodexHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: { openai: { adapter: "openai-chat", baseUrl: "https://api.example.test/v1", liveModels: false, models: ["fresh-model"] } } }).then(res => console.log(JSON.stringify(res)));
    `);
    expect(r.status).toBe(0);
    const slugs = (JSON.parse(readFileSync(catalogPath, "utf8")).models as Array<{ slug: string }>).map(m => m.slug);
  ...
  test("authoritative empty providers drop their own rows and deleted-provider ghosts", () => {
    ...
    const r = runScript(codexHome, opencodexHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: { openai: { adapter: "openai-chat", baseUrl: "https://api.example.test/v1", liveModels: false, models: [] } } }).then(res => console.log(JSON.stringify(res)));
    `);
    expect(r.status).toBe(0);
    const slugs = (JSON.parse(readFileSync(catalogPath, "utf8")).models as Array<{ slug: string }>).map(m => m.slug);
    expect(slugs).not.toContain("future-grok/old-model");
  ...
  test("drops legacy combo-alias ghost rows in both gather branches", () => {
    const legacyComboAlias = { ...routedEntry("vendor/fast", 5), description: "Routed via opencodex → combo (combo).", owned_by: "combo" };
    const catalogPath = join(codexHome, "catalog.json");
    const seed = () => writeFileSync(catalogPath, JSON.stringify({ models: [nativeEntry("gpt-5.5", 0), legacyComboAlias, routedEntry("cursor/composer-2.5", 6)] }, null, 2) + "\n");
    writeFileSync(join(codexHome, "config.toml"), 'model_catalog_json = "catalog.json"\n', "utf8");
    seed();
    const partial = runScript(codexHome, opencodexHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: { combo: { adapter: "openai-chat", baseUrl: "https://api.example.test/v1", liveModels: false, models: ["fresh-model"] } } }).then(res => console.log(JSON.stringify(res)));
    `);
    expect(partial.status).toBe(0);
    let slugs = (JSON.parse(readFileSync(catalogPath, "utf8")).models as Array<{ slug: string }>).map(m => m.slug);
    ...
    const empty = runScript(codexHome, opencodexHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: { combo: { adapter: "openai-chat", baseUrl: "https://api.example.test/v1", liveModels: false, models: [] } } }).then(res => console.log(JSON.stringify(res)));
    `);
    expect(empty.status).toBe(0);
    slugs = (JSON.parse(readFileSync(catalogPath, "utf8")).models as Array<{ slug: string }>).map(m => m.slug);
    expect(slugs).not.toContain("vendor/fast");
  ...
  test("replaces existing routed entries for providers present in the current sync config", () => {
    ...
    const r = runScript(codexHome, opencodexHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: { cursor: { adapter: "cursor", baseUrl: "https://api2.cursor.sh", liveModels: false, models: ["composer-2.5"] } } }).then(res => console.log(JSON.stringify(res)));
    `);
    expect(r.status).toBe(0);
    const slugs = (JSON.parse(readFileSync(catalogPath, "utf8")).models as Array<{ slug: string }>).map(m => m.slug);
    expect(slugs).toContain("cursor/composer-2.5");
  ...
  test("an identical resync leaves the catalog file untouched, a real change still writes", () => {
    ...
    const r = runScript(codexHome, opencodexHome, `
      ...
      (async () => {
        ...
        console.log(JSON.stringify({ firstWritten: first.catalogWritten, secondWritten: second.catalogWritten, secondAdded: second.added, identicalResyncKeptMtime: afterFirst === afterSecond, thirdWritten: third.catalogWritten, realChangeBumpedMtime: statSync(path).mtimeMs > changedAt }));
      })();
    `);
    expect(r.status).toBe(0);
  ...
  test("the no-op guard compares bytes, so a malformed byte decoding to U+FFFD is still repaired", () => {
    ...
    const r = runScript(codexHome, opencodexHome, `
      ...
      (async () => {
        ...
        const corrupted = Buffer.concat([converged.subarray(0, at), Buffer.from([0x80]), converged.subarray(at + replacement.length)]);
